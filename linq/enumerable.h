...
namespace sb {
    ...
    public:
        ...
        enumerable_exception(std::string&& msg) : m_emsg(std::move(msg)) 
        {
        }
    ...
    private:
        std::shared_ptr<typename iterator_wrap<Type>::placeholder> m_left_begin;
        std::shared_ptr<typename iterator_wrap<Type>::placeholder> m_left_end;
        std::shared_ptr<typename iterator_wrap<Type>::placeholder> m_right_begin;
        bool m_left;

    public:
        template <typename LeftIterator, typename RightIterator>
        concat_iterator(const LeftIterator& left_begin, const LeftIterator& left_end, const RightIterator& right_begin) :
            m_left_begin(std::make_shared<typename iterator_wrap<Type>::template holder<LeftIterator>>(left_begin)),
            m_left_end(std::make_shared<typename iterator_wrap<Type>::template holder<LeftIterator>>(left_end)),
            m_right_begin(std::make_shared<typename iterator_wrap<Type>::template holder<RightIterator>>(right_begin)),
            m_left(left_begin != left_end)
        {
        }
        ...
        Self& operator++()
        {
            if (m_left) {
                ...
                if (m_left_begin->equals(m_left_end)) {
                    m_left = false;
                }
        ...
        Type operator*() const
        {
            return m_left ? m_left_begin->value() : m_right_begin->value();
        }
        ...
        bool operator==(const Self& rhs) const
        {
            ...
            return m_left ? m_left_begin->equals(rhs.m_left_begin) : m_right_begin->equals(rhs.m_right_begin);
        }
    ...
    template <typename LeftIterator, 
              ...
              typename Type = typename recover_type<typename std::iterator_traits<LeftIterator>::value_type>::type>
    concat_iterator<Type> make_concat_iterator(const LeftIterator& left_begin, const LeftIterator& left_end, const RightIterator& right_begin)
    {
        return concat_iterator<Type>(left_begin, left_end, right_begin);
    }
    ...
    public:
        ...
        template <typename Functor, 
                  typename Result = enumerable<typename functor_retriver<decltype(&Functor::operator())>::type>>
        ...
        template <typename Functor>
        auto select_many(const Functor& selector) const->
            enumerable<decltype(*((typename functor_retriver<decltype(&Functor::operator())>::type*)0)->begin())>
        {
        ...
        bool sequence_equal(const Iterator& right_begin, const Iterator& right_end) const
        {
            ...
            while (left_begin != left_end && rbegin != rend) {
                if (*left_begin++ != *rbegin++) {
                    return false;
                }
            }

            return left_begin == left_end && rbegin == rend;
        }
